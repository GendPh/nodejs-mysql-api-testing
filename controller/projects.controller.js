const pool = require("../database/index");

const table = "projects";

const projectsController = {
  getAll: async (req, res) => {
      try {
          const [rows, fields] = await pool.query(`select * from ${table}`)
          res.json({
              data: rows
          })
      } catch (error) {
          console.log(error)
          res.json({
              status: "Fail to get all projects"
          })
      }
  },
  getById: async (req, res) => {
      try {
          const { id } = req.params
          const [rows, fields] = await pool.query(`select * from ${table} where id = ?`, [id])
          res.json({
              data: rows
          })
      } catch (error) {
          console.log(error)
          res.json({
              status: `failed to get project with the id ${id}`
          })
      }
  },
  create: async (req, res) => {
      try {
          const { title, content } = req.body
          const sql = `insert into ${table} (title, content) values (?, ?)`
          const [rows, fields] = await pool.query(sql, [title, content])
          res.json({
              data: rows
          })
      } catch (error) {
          console.log(error)
          res.json({
              status: "error"
          })
      }
  },
  update: async (req, res) => {
      try {
          const { title, content } = req.body
          const { id } = req.params
          const sql = `update ${table} set title = ?, content = ? where id = ?`
          const [rows, fields] = await pool.query(sql, [title, content, id])
          res.json({
              data: rows
          })
      } catch (error) {
          console.log(error)
          res.json({
              status: "error"
          })
      }
  }, 
  delete: async (req, res) => {
      try {
          const { id } = req.params
          const [rows, fields] = await pool.query(`delete from ${table} where id = ?`, [id])
          res.json({
              data: rows
          })
      } catch (error) {
          console.log(error)
          res.json({
              status: "error"
          })
      }
  }

}

module.exports = postsController

